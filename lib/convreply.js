...
module.exports = function() {
    ...
    return function(msg, id, rep) {
        ...
        if (! rep && last) {
            ...
            let s = m[0];
            if (m.match(/\d{4}/)) {
                reply = { type: 'ankan', actor: id, consumed: [] };
                reply.consumed = m.match(/\d/g).map(n => pai(s, n));
            }
            else {
                reply = { type: 'kakan', actor: id, pai: '', consumed: [] };
                reply.pai = pai(s, m.slice(-1));
                reply.consumed
                        = m.match(/(?<![\+\=\-])\d/g).map(n => pai(s, n));
            }
        }
            ...
            if (msg.type == 'tsumo') {
                reply = { type: 'ryukyoku', actor: id, reason: 'kyushukyuhai' };
            }
        }
        return reply;
    }
